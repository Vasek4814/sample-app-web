...
const validUsers = [{ userNames: 'standard_user', password: 'secret_sauce' }];
...
test.describe('Успешная авторизация с валидными учётными данными', () => {
  validUsers.forEach(({ userNames, password }) => {
    test(`Успешный вход под пользователем ${userNames}`, async ({ page }) => {
      await test.step('Ввод логина и пароля', async () => {
        await page.locator('#user-name').fill(userNames);
