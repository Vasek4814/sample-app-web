...
test.describe('Авторизация', () => {
  ...
  test('Ошибка "Username is required" при пустых полях', async ({ loginPage }) => {
    await loginPage.open();
  ...
  test('Ошибка для заблокированного пользователя locked_out_user', async ({ loginPage }) => {
    await loginPage.open();
  ...
  test('Сообщение об ошибке закрывается по клику на иконку X', async ({ loginPage }) => {
    await loginPage.open();
    await loginPage.login(USERS.locked);
